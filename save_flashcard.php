...
if ($action === 'history') {
    // Trả về lịch sử 30 ngày gần nhất
    $rows = $db->query("
        SELECT date, topic,
               COUNT(*) as total,
               SUM(CASE WHEN rating IN ('good','easy') THEN 1 ELSE 0 END) as known
...
$stmt = $db->prepare('INSERT INTO flashcard_history (user_id, date, topic, word, phonetic, word_type, meaning, rating)
    VALUES (:uid, :date, :topic, :word, :phonetic, :type, :meaning, :rating)');
